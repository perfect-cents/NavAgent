...
from navhelpers import iterate
...
def cmd_angle_deviate(anlge):
    current_waypoint = LocationGlobalRelative(CMD_LIST[CMDS.next].y,
                                              CMD_LIST[CMDS.next].x,
                                              CMD_LIST[CMDS.next].z)

    current_bearing = get_bearing(LOC.global_relative_frame,
                                  current_waypoint)
    bearing = current_bearing + angle
    
    return loc_to_cmd(get_polar_location(bearing, 2000, LOC.global_relative_frame))

def cmd_simple_deviate(deviation):
    """ A simple default deviation procedure.
    """
    dev_cmd_list = []
    ...
    deviation_bearing = get_bearing(LOC.global_relative_frame,
                                    deviation)

    relative_deviation_bearing = deviation_bearing - current_bearing \
                                 - (360 if deviation_bearing \
                                           - current_bearing \
                                    > 180 else 0)
    clockwise = True if relative_deviation_bearing > 0 else False

    dev_cmd_list.append(cmd_takeoff(10))  # TODO : CHANGE

    dev_cmd_list.extend(cmd_polygon(deviation, 25, 8, clockwise,
                                (deviation_bearing + -90 if clockwise else 90)
                                )
                        )

    dev_cmd_list.append(loc_to_cmd(current_waypoint))
    dev_cmd_list.append(dev_cmd_list[-1])

    return dev_cmd_list

def cmd_deviate(deviation):
    return cmd_simple_deviate(deviation)
...
def bla_angle_deviate(angle):
    cmd_list = []
    
    cmd_list.append(cmd_takeoff(10))
    cmd_list.append(cmd_angle_deviate(angle))
    cmd_list.append(cmd_list[-1])
    
    auto_cmd_wp_nbr = CMDS.next
    ...
    add_cmds(CMD_LIST)
    CMDS.next = auto_cmd_wp_nbr + 1

def bla_deviate(deviation):
    """ docstring! docstring? docstring...
    """

    dev_cmds = []

    if CMDS.next + 1 == len(CMD_LIST):
        dev_cmds = cmd_simple_deviate(deviation)
    else:
        dev_cmds = cmd_deviate(deviation)

    auto_cmd_wp_nbr = CMDS.next

    print "makeing deviation..."

    add_cmds(cmd_deviate(deviation))

    print "uploaded deviation..."
...
def main():
    ...
    CMD_LIST.append(loc_to_cmd(VEHICLE.home_location))  # TODO : proper alt?
    # CMD_LIST.append(CMD_LIST[-1])
    ...
    DOG_LOC = get_ortho_location(60, 50, VEHICLE.home_location)
    dun_it = False

    print "Mission Started!"
    while VEHICLE.system_status == "ACTIVE":
        if not dun_it and get_distance(LOC.global_relative_frame, DOG_LOC) < 40:
            deviate(DOG_LOC)
            dun_it = True
